...
const slotStateClasses: Record<Slot["status"], string> = {
  available: "bg-brand-muted/80 text-brand border-transparent",
  occupied: "bg-surface border border-transparent text-text-muted opacity-60",
  reserved: "bg-warning/10 text-warning border-warning/40",
};
        ...
        {slots.map((slot) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs font-normal text-text-muted">${slot.pricePerHour.toFixed(2)}/h</p>
            </motion.button>
          );
        })}
      </div>
